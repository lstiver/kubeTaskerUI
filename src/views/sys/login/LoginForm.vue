<template>
  ...
  <Form class="p-4 enter-x" :model="formData" :rules="getFormRules" ref="formRef" v-show="getShow"
    ...
    <FormItem v-if="formData.msgType !== 'captcha'" name="target" class="enter-x">
      <Input size="large" v-model:value="formData.target" v-model:placeholder="emailOrPhonePlaceholder" />
    </FormItem>

    <FormItem v-if="formData.msgType !== 'captcha'" name="captchaVerified" class="enter-x">
      <CountdownInput size="large" v-model:value="formData.captchaVerified" :count="60"
        :placeholder="t('sys.login.captcha')" :send-code-api="handleSendCaptcha" />
    </FormItem>

    <FormItem name="account" v-if="formData.msgType === 'captcha'" class="enter-x" :rules="[{ required: true, max: 30 }]">
      <Input size="large" v-model:value="formData.account" :placeholder="t('sys.login.username')" class="fix-auto-fill" />
    </FormItem>

    <FormItem name="password" class="enter-x" v-if="formData.msgType === 'captcha'"
      :rules="[{ required: true, min: 6, max: 30 }]">
      <InputPassword size="large" visibilityToggle v-model:value="formData.password"
        :placeholder="t('sys.login.password')" />
    </FormItem>

    <FormItem name="captcha" v-if="formData.msgType === 'captcha'" class="enter-x" :rules="[{ required: true, len: 5 }]">
      <Input size="large" v-model:value="formData.captcha" :placeholder="t('sys.login.captcha')" class="fix-auto-fill">
      <template #suffix>
